...
for index, record in enumerate(records[:124]):
    if len(record) == 8 and index > 104: # Want to find a headline for this record
        ...
        response = requests.get(url, params={"source-countries": "us", "text": headline_query, "api-key": api_keys["world_news"], "language":"en"})
