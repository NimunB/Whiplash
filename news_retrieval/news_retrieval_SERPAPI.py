...
good_words = ["success", "charity", "win", "happy", "exciting", "hardworking", "brilliant", "feat", "reward", "good", "praise", "celebrate", "pride", "joy", "goodwill", "helpful", "reuinte", "good samaritan", "feeding", "hope", "startup", "funding", "art", "creativity", "hero"]
...
#for index, record in enumerate(records[:1608]): # up to 1 less
    if len(record) == 8: # and index > 1400: # at + 4
        ...
        response = requests.get(url, params={"engine": "google_news", "q": headline_query, "api_key": api_keys["serpapi"]})
        ...
        if response.status_code == 200:
            ...
            # Check if the key 'news_results' exists in the dictionary before accessing it
            if 'news_results' in data:
                # Looking at the first 5 results
                for article in data["news_results"][:min(5, len(data["news_results"]))]: 
